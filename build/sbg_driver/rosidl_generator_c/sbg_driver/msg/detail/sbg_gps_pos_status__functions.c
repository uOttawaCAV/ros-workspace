  ...
  sbg_driver__msg__SbgGpsPosStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    sbg_driver__msg__SbgGpsPosStatus * data =
      (sbg_driver__msg__SbgGpsPosStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!sbg_driver__msg__SbgGpsPosStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          sbg_driver__msg__SbgGpsPosStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
