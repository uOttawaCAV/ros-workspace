  ...
  sbg_driver__msg__SbgShipMotionStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    sbg_driver__msg__SbgShipMotionStatus * data =
      (sbg_driver__msg__SbgShipMotionStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!sbg_driver__msg__SbgShipMotionStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          sbg_driver__msg__SbgShipMotionStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
