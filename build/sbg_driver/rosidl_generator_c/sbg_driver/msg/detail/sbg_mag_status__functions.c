  ...
  sbg_driver__msg__SbgMagStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    sbg_driver__msg__SbgMagStatus * data =
      (sbg_driver__msg__SbgMagStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!sbg_driver__msg__SbgMagStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          sbg_driver__msg__SbgMagStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
