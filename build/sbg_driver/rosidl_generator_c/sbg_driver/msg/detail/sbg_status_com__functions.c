  ...
  sbg_driver__msg__SbgStatusCom__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    sbg_driver__msg__SbgStatusCom * data =
      (sbg_driver__msg__SbgStatusCom *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!sbg_driver__msg__SbgStatusCom__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          sbg_driver__msg__SbgStatusCom__fini(&data[i]);
        }
        free(data);
        return false;
      }
